...
public class CatServeService extends IntentService {
    ...
    public static void startActionSaveCats(final Context context, final int from, final int to) {
    ...
    protected void onHandleIntent(final Intent intent) {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.O) {
            NotificationChannel channel = new NotificationChannel(CHANNEL_ID_SAVE_CAT, getString(R.string.save_the_cats), NotificationManager.IMPORTANCE_LOW);
        ...
        handleActionSaveCats(builder, from, to);
    }

    private void handleActionSaveCats(NotificationCompat.Builder builder, final int from, final int to) {
        ...
        List<Future<String>> list = new ArrayList<Future<String>>(MAX);
        List<String> pathList = new ArrayList<>(MAX);
        ...
        for (int count = 0; count < list.size(); count++) {
            try {
                pathList.add(list.get(count).get());
        ...
        String[] pathArray = pathList.stream().filter(Objects::nonNull).toArray(String[]::new);
        MediaScannerConnection.scanFile(getApplicationContext(), pathArray, null, null);
    ...
    private static class CatSaver implements Callable<String> {
